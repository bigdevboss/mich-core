...
unsigned int paging_clone_user(unsigned int dst_pd_phys, unsigned int src_pd_phys) {
    for (int i = USER_PD_INDEX_START; i < 512; i++) {
        ...
        for (int j = 0; j < PAGE_TABLE_ENTRIES; j++) {
            ...
            unsigned char *from = (unsigned char *)paging_kmap_a(sp & 0xFFFFF000);
            unsigned char *to = (unsigned char *)paging_kmap_b(np);
            for (unsigned int b = 0; b < PAGE_SIZE; b++) to[b] = from[b];
        }
    }
    return 1;
}
