...
void gfx_clear(void) {
    if (screen_bpp == 32) {
        for (unsigned int y = 0; y < screen_height; y++) {
            unsigned int *row = &framebuffer[y * (screen_pitch / 4)];
            for (unsigned int x = 0; x < screen_width; x++) row[x] = 0x00000000;
        }
    } else if (screen_bpp == 24) {
        unsigned char *fb_bytes = (unsigned char *)framebuffer;
        for (unsigned int y = 0; y < screen_height; y++) {
            unsigned int off = y * screen_pitch;
            for (unsigned int x = 0; x < screen_width * 3; x++) fb_bytes[off + x] = 0;
        }
    }
    cursor_x = 0;
    cursor_y = 0;
