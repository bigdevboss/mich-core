...
case "$profile" in
    ...
    pcie)
        set -- -machine q35
        expanded_image="$(mktemp)"
        cp "$image" "$expanded_image"
        truncate -s 16M "$expanded_image"
        image="$expanded_image"
        ;;
    iommu)
        set -- -machine q35 -device intel-iommu,intremap=off -device edu
        expanded_image="$(mktemp)"
        cp "$image" "$expanded_image"
        truncate -s 16M "$expanded_image"
        image="$expanded_image"
        ;;
    amd-iommu)
        set -- -machine q35 -device amd-iommu,intremap=off
        expanded_image="$(mktemp)"
        cp "$image" "$expanded_image"
        truncate -s 16M "$expanded_image"
        image="$expanded_image"
